...
void geneticAlgorithm::initializePop(int rnk){

	_population = InitializePopulation(_npopulation, _ncities, rnk);
	
}

void geneticAlgorithm::replacePath(vector<int> newPath){
	
	for(int i=0; i<_ncities; i++){
		
		_population[0][i] = newPath[i];
	}
}

void geneticAlgorithm::pointReplace(int n,int path, int cit){

	_population[path][cit] = n;
}
