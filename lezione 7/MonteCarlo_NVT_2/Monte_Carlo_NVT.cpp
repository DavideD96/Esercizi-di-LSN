  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      ...
      if(istep%10 == 0){
        ...
        nconf += 1;
	//cout<<"config "<< istep <<endl;
      }
    }
    cout<<"prima di averages"<< endl;
...
//update of the histogram of g(r)
     ...
     bin_number = binSearcher(bin_size, nbins, dr);
     if(bin_number != -1){
         walker[bin_number+2] = walker[bin_number+2] + 2;
     }
...
// contribution to energy and virial
  ...
  press = rho * temp + (walker[iw] + (double)npart * ptail) / vol;

  ofstream Energy, Pressure;
  Energy.open("en_pot.dat", ios::app);
  Pressure.open("pressure.dat", ios::app);

  Energy << walker[iv]/(double)npart << endl;
  Pressure << press << endl;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
	...
	walker[i+2] = (double)walker[i+2]/normalization; //g(r)

        glob_av[i+2] += walker[i+2];
        glob_av2[i+2] += pow(walker[i+2],2);
        err_gdir[i] = Error(glob_av[i+2],glob_av2[i+2],iblk);
    }
...
//g(r)
    ...
    for(int k=0; k<nbins; k++){
        Gofr << setw(wd) << walker[k+2];
        Gave << setw(wd) << glob_av[k+2]/(double)iblk << setw(wd) << err_gdir[k] << "    ";
    }
    Gofr << endl;
    Gave << endl;
